...
import { cardClass, helperTextClass, sectionTitleClass } from '../constants/ui'
import {
  ACTIVE_SIMULATION_KEY,
  DEFAULT_SIMULATION_DATA,
  EMPLOYEE_CONTRIB_RATE,
  EMPLOYER_CONTRIB_RATE,
  SIMULATIONS_KEY,
  annualEmployerContribAtom,
  annualGrossSalaryAtom,
  annualNetSalaryAtom,
  annualTurnoverAtom,
  corporateTaxAtom,
  corporateTaxNormalRateAtom,
  corporateTaxReducedRateAtom,
  corporateTaxThresholdAtom,
  currencyFormatter,
  daysWorkedAtom,
  deductibleChargesAtom,
  dividendFlatTaxRateAtom,
  distributableResultAtom,
  monthlyIncomeTaxRateAtom,
  monthlyNetSalaryAtom,
  netDividendsAtom,
  netSalaryAfterWithholdingAtom,
  resultBeforeTaxAtom,
  tjmAtom,
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="flex flex-col gap-3">
              {deductibleCharges.map((charge) => (
                ...
                >
                  ...
                  className="w-3/4 flex-1 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/40 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
                  />
                  <input
                  value={charge.amount || ''}
                  inputMode="decimal"
                  onChange={(event) => updateCharge(charge.id, 'amount', event.target.value)}
                  className="w-1/4 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/40 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
                  placeholder="€"
                  />
                  <button
                  ...
                  >
                  <span className="sr-only">Supprimer</span>&times;
                  </button>
                </div>
              ))}
        ...
        <div className={cardClass}>
          <div className="flex flex-col gap-3">
            ...
            <InlineDetail label="IS réduit (0€ → 42 500€)" value={formatRate(corporateTaxReducedRate)} />
            <InlineDetail label="IS normal (> 42 500€)" value={formatRate(corporateTaxNormalRate)} />
            <InlineDetail label="PFU dividendes" value={formatRate(dividendFlatTaxRate)} />
            <p className={helperTextClass}>
